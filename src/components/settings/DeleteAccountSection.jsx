...
export function AdminDashboard() {
  ...
  const deleteUser = async () => {
    ...
    try {
      setLoading(true);
      let response;
      if (userToDelete.type === 'advertiser') {
        response = await axios.post("/api/routes/v1/authRoutes?action=deleteAdvertiserUser", {
          id: userToDelete.id
        });
      } else {
        response = await axios.post("/api/routes/v1/authRoutes?action=deleteConsumerUser", {
          id: userToDelete.id
        });
      }
  ...
  const handleDeleteUser = (userId, userType) => {
    setUserToDelete({ id: userId, type: userType });
  ...
  const userColumns = [
    ...
    { label: "TOTAL WITHDRAW", key: "withdraw" },
    {
      ...
      render: (item) => (
        <button
          onClick={() => handleDeleteUser(item.id, 'consumer')}
          className="text-xs font-medium px-3 py-1 rounded-full bg-red-100 text-red-700 hover:bg-red-200"
        >
          Delete
        </button>
  ...
  const advertiserColumns = [
    ...
    { label: "PROFILE TYPE", key: "profileType" },
    {
      ...
      render: (item) => (
        <button
          onClick={() => handleDeleteUser(item.id, 'advertiser')}
          className="text-xs font-medium px-3 py-1 rounded-full bg-red-100 text-red-700 hover:bg-red-200"
        >
          Delete
        </button>
  ...
  return (
    <main className="flex h-auto min-h-screen w-full flex-col bg-[var(--bg-color-off-white)]">
      ...
      <ConfirmationModal
        ...
        description={`Are you sure you want to permanently delete this ${userToDelete?.type === 'advertiser' ? 'advertiser' : 'consumer'} user? This action cannot be undone.`}
        confirmButtonText={loading ? "Deleting..." : "Delete User"}
